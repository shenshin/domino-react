...
const dominoSlice = createSlice({
  ...
  reducers: {
    ...
    setGameStockCoords: (state, { payload: { tile, location } }) => {
      const tileInStock = state.stock.find((t) => t.id === tile.id);
      if (tileInStock) {
        tileInStock.location.gameStock = location;
      }
    },
    setPlayerStockCoords: (state, { payload: { tile, location } }) => {
      const currPlayer = state.players.find((p) =>
        p.stock.some((t) => t.id === tile.id),
      );
      if (currPlayer) {
        const playersTile = currPlayer.stock.find((t) => t.id === tile.id);
        if (playersTile) {
          playersTile.location.playerStock = location;
        }
      }
    },
    ...
    drawTileToAI: (state) => {
      if (state?.stock.length > 0) {
        state.players[1].stock.push(state.stock.splice(0, 1)[0]);
      }
    },
    drawTileToUser: (state) => {
      if (state?.stock.length > 0) {
        state.players[0].stock.push(state.stock.splice(0, 1)[0]);
      }
    },
    drawTileToPlayline: (state) => {
      if (state?.stock.length > 0) {
        state.playline.push(state.stock.splice(0, 1)[0]);
      }
    },
    ...
    userMakesMove: (state, { payload: { tile, position } }) => {
      ...
      if (user.stock.length === 0) {
        ...
        user.stock.push(state.stock.splice(0, 1)[0]);
      }
    },
    ...
    aiMakesMove: (state) => {
      ...
      const matchingTiles = ai.stock.filter((t) =>
        getNumbers(t).some((n) => playlineEdges.includes(n)),
      );
      if (matchingTiles.length > 0) {
        ...
        if (ai.stock.length === 0) {
          ...
          // add new tile from stock
          ai.stock.push(state.stock.splice(0, 1)[0]);
        }
      } else {
        ai.missedLastMove += 1;
      }
